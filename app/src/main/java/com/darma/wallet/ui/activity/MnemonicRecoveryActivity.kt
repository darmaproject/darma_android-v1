...
import kotlinx.android.synthetic.main.activity_mnemonic_recovery.*

class MnemonicRecoveryActivity : BaseActivity() {
    override fun layoutId(): Int {
        return R.layout.activity_mnemonic_recovery
    }
    ...
    override fun initView() {
        ...
        et_mnemonic.editText?.addTextChangedListener(object : TextWatcher{
            override fun afterTextChanged(s: Editable?) {
                btn_recovery_wallet.setProgress(0);
            }
    ...
    private fun createWallet() {
        ...
        var key = et_mnemonic.editText?.text.toString()
        ...
        try {
            WalletManager.getInstance().recoveryWalletByMnemonic(name, pwd,height,key)
    ...
    private fun checkInput(): Boolean {
        ...
        if (StringUtils.isEmpty(key)) {
            et_mnemonic.error = getString(R.string.str_please_input_mnemonic)
            checkEmpty = true
        }
        if (checkEmpty) {
            return false
        ...
        if (!CheckUtils.isLegalMnemonicWords(key)) {
            et_mnemonic.error = getString(R.string.str_mnemonic_error)
            return false
        }
        return true
    }
